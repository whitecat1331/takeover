...
services = {
    ...
    "Pantheon": {
        "error": r"The gods are wise, but do not know of the site which you seek."
    },
    "Tumbler": {
    ...
    "Campaignmonitor": {
        "error": r"Double check the URL or <a href=\"mailto:help@createsend.com"
    },
    "Acquia": {
        "error": r"The site you are looking for could not be found.|If you are an Acquia Cloud \
customer and expect to see your site at this address"
    },
    "Proposify": {
...
def find(status, content, ok):
    for service in services:
        for values in services[service].items():
            if re.findall(str(values[1]), str(content), re.I) and int(status) in range(
                201 if ok is False else 200, 599
            ):
